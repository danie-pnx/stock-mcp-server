...
Serving as the server instance defined in [server.py](https://github.com/danie-pnx/stock-mcp-server/blob/main/server.py), this project:
...
The server lifecycle and request processing pipeline are implemented within [server.py](https://github.com/danie-pnx/stock-mcp-server/blob/main/server.py). The pipeline consists of the following 5 phases:
...
2. **Tool Registration (yfinance Integration):**
   Defines and registers the [get_ticker_news](https://github.com/danie-pnx/stock-mcp-server/blob/main/server.py#L18) tool with the `@mcp.tool()` decorator, allowing it to accept a ticker symbol and query `yfinance` to retrieve stock headlines.
